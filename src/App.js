...
class Mine extends Component {
  ...
  submit(e) {
      e.preventDefault();
      if (this.state.answer === "Moon" || this.state.answer === "moon") {
...
class Ledger extends Component {
  render() {
    ...
    for (var i = 0; i < ledger.length; i++) {
      all.push(<tr key = {i}><td>{ledger[i].action}</td><td>{ledger[i].amount}</td>
      <td>{ledger[i].value}</td><td><NavLink to = 
      {"/transaction/" + ledger[i].id}><button className = "btn btn-info">
